...
// ===== TOOLTIP TOGGLE ON MOBILE =====
document.addEventListener('click', e => {
  const btn = e.target.closest('.code-info-btn');
  if (btn) {
    const tooltip = btn.parentElement.querySelector('.code-info-tooltip');
    if (tooltip) {
      const isOpen = tooltip.classList.contains('open');
      // Close all other tooltips first
      document.querySelectorAll('.code-info-tooltip.open').forEach(t => t.classList.remove('open'));
      if (!isOpen) tooltip.classList.add('open');
    }
    return;
  }
  // Close tooltips when clicking outside
  if (!e.target.closest('.code-info-tooltip')) {
    document.querySelectorAll('.code-info-tooltip.open').forEach(t => t.classList.remove('open'));
  }
});
